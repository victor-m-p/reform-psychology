...
m_prior <- fit_model(
  ...
  prior = p,
  sample_prior = "only",
  file = "/work/50114/MAG/modeling/replication/models/main_model/m_prior"
)
...
pp_check(m_prior, ndraws = 50) + xlim(0, 50)
...
m_post <- fit_model(
  ...
  prior = p,
  sample_prior = TRUE,
  file = "/work/50114/MAG/modeling/replication/models/main_model/m_post"
)
...
--> we regularize / make the effect smaller (should see in prior robustness plot)
--> could join plots (i.e. control and experiment for each). 
...
sometimes 1 or 2 instead of 0 (just very low).
so no need to produce more 0s. 
...
-- could aggregate 0, 1, 2 (we do have this problem, but maybe not that problematic).
...
Looks pretty good (really good plot)
...
# for log of teamsize: log(2) - log(1) for scaling

hypothesis(model, "exp(condition1) > exp(condition2)")
hypothesis(model, "exp(condition1 + condition1:year * scaling) - exp(condition1) > the same for experiment")

```

### outcome scale for intercept (difference)
---> actually a pretty wide prior we have. 

```{r}

h_intercept_outcome <- hypothesis(
  m_post, 
  "exp(condition_fctexperiment) < exp(condition_fctcontrol)", 
  alpha = 0.05)
plot(h_intercept_outcome)

```

```{r}
ci_lower <- h_intercept_outcome$hypothesis$CI.Lower
ci_upper <- h_intercept_outcome$hypothesis$CI.Upper
ci_est <- h_intercept_outcome$hypothesis$Estimate
```


### difference between going from 0 co-authors to 1 co-author. 
--> could also do other differences... 
...
# isolated difference: 
moving from 0 co-authors to 1 co-author: 
* very significant (p = 0) 
* small effect (0.5 citations difference in gain) 

```{r}

h_teamsize_outcome <- hypothesis(
  m_post_inf, 
  "exp(condition_fctexperiment:log_teamsize * 0.69) < exp(condition_fctcontrol:log_teamsize * 0.69)",
  alpha = 0.5
)
...
# putting it together: 
what does it mean that this is not significant then? 

```{r}

# combines the two: 
# is a bit complicated
h_intercept_teamsize_outcome <- hypothesis(
  m_post_inf, 
  "exp(condition_fctexperiment + condition_fctexperiment:log_teamsize * 0.69) - exp(condition_fctexperiment) < exp(condition_fctcontrol + condition_fctcontrol:log_teamsize * 0.69) - exp(condition_fctcontrol)",
  alpha = 0.05
)
plot(h_intercept_teamsize_outcome)
...
we have high pareto k in all models. 
--> even if we remove just one it will influence. 
--> maybe overly influenced by those (ideally we would observe more high-citation)
---> or include predictors that explain why they are exceptional. 

```{r}

d %>% 
  mutate(k = m_post_inf$criteria$loo$diagnostics$pareto_k) %>% 
  filter(k > .7) %>% 
  select(c_5, log_teamsize, condition_fct, id_match, k)
