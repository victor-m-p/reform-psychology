...
#outpath <- "/work/50114/MAG/fig/modeling/replication_fos/results/"
...
library(pacman)
p_load(tidyverse, brms, ggthemes, bayesplot, cowplot, tidybayes, modelr, latex2exp, ggpubr, glue)
...
ggsave(filename = paste0(outpath, tag, "main_intercept_predict.pdf"),
       plot = p_grid,
       width = 8,
       height = 7)
...
plt_year <- function(draws, d, clrs, ylim, y_min){
...
p_year <- plt_year(d_year_mean, d, clrs, ymax, x_min_int)
...
ggsave(filename = paste0(outpath, tag, "main_interaction_effs.pdf"),
       plot = p_grid,
       width = 8,
       height = 5)
