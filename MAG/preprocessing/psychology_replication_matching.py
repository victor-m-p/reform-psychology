...
psych_control_clean = clean_records(psych_control_2016, "PaperIdControl")
psych_experiment_clean = clean_records(psych_replication_2016, "PaperIdExperiment")
...
def get_matches(d_exp, d_control):
    ...
    for i in tqdm(range(len(d_exp))):
        ...
        used_index = d_asof["PaperIdControl"].item()    
        d_control = d_control[d_control["PaperIdControl"] != used_index]
...
def check_matching(psych_joined, psych_control):
    ...
    psych_control_dates = psych_control.rename(columns={'Date': 'DateControl'})[["PaperIdControl", "DateControl"]]
