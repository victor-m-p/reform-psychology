...
def get_keyword_subset(df_meta, df_paa, fos, keyword): 
    ...
    df_keyword = df_keyword[["PaperId", "Date", "n_authors", "NormalizedName", "DocType", "PaperTitle"]] # DocType added 

    # control data
    df_control = filtered_df[~filtered_df["PaperId"].isin(df_keyword["PaperId"])]
    df_control = df_control[["PaperId", "Date", "n_authors", "NormalizedName", "DocType", "PaperTitle"]]
...
def get_matches(d_exp, d_control):
    ...
    for i in tqdm(range(len(d_exp))):
        ...
        d_asof = pd.merge_asof(
            ...
            by = ['n_authors', 'NormalizedName', 'DocType'], # has to be equivalent
            direction = 'nearest')
...
def main(inpath, field, query, outpath): 
    print(f"--- running: query main ---")
    print(f"--> field: {field}")
    print(f"--> query: {query}")
    ...
    print(f"--> cleaning & keywords")
    # clean files
    df_meta_clean, df_paa_clean, df_ref_clean = clean_data(df_meta, df_paa, df_ref)

    # match & keyword
    df_keyword_2016, df_control_2016 = get_keyword_subset(df_meta_clean, df_paa_clean, field, query)

    # clean records 
    df_control_clean = clean_records(df_control_2016, "PaperId_control", "PaperTitle_control")
    df_experiment_clean = clean_records(df_keyword_2016, "PaperId_experiment", "PaperTitle_experiment")
...
if __name__ == '__main__':
    ...
    ap.add_argument("-q", "--query", required=True, type=str, help="querry to search for in titles, e.g. replicat")
    ap.add_argument("-o", "--outpath", required=True, type=str, help="path to folder for output csv")
    args = vars(ap.parse_args())

    main(inpath = args["inpath"], field = args["field"], query = args["query"], outpath = args["outpath"])
