...
def main():
    ...
    nx.draw_networkx(
        ...
        edge_color="gray",
        nodelist=keys,
        #node_size=[v * 10 for v in engagement] ### just commented out ###
        )
    plt.tight_layout()
    plt.savefig(f'fig/{os.path.basename(fname).split(".")[0]}_nmf_msgidx.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'fig/{os.path.basename(fname).split(".")[0]}_nmf_widx.png')
    plt.close()
