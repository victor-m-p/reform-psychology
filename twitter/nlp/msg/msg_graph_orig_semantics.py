...
G2 = nx.read_gml("/work/cn-some/msg/data/diplomat_orig_wmax.gml", destringizer=int) # takes some time to load
...
nx.draw_networkx_nodes(G2, pos, node_size = [x/30 for x in node_size], node_color = "#210070")
nx.draw_networkx_edges(G2, pos, edge_color = "m", width = [x/300 for x in edge_width], alpha = [x/max_width for x in edge_width])
...
plt.axis("off")
pos = nx.spring_layout(G2, seed=7) # can be tweaked individually. 
pos[11] += (0.5, 0) # business-travel
pos[12] += (0, 0.05)
nx.draw_networkx_nodes(G2, pos, node_size = [x/25 for x in node_size], node_color = "#210070")
nx.draw_networkx_edges(G2, pos, edge_color = "m", width = [x/300 for x in edge_width], alpha = [x/max_width for x in edge_width])
label_options = {"ec": "k", "fc": "white", "alpha": 0.7}
nx.draw_networkx_labels(G2,pos,labels=labeldict,font_size=18, bbox=label_options)
plt.tight_layout()
plt.savefig("/work/cn-some/msg/fig/diplomat_orig_spring_tweaked.png")
