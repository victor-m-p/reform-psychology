...
def dist_prune(delta, w = 2): # changed from 2. 
...
# set stuff manually
fname = '../data/msg/meta_msg.csv' # file name 
W = np.loadtxt("../data/msg/meta_msg_W.txt") # crazy metric

D = pairwise_distances(W, metric="cosine")
print(D.shape)

# prune
D = dist_prune(D, w=2.5)

idxs = list()
## remove no relations
for (i, d) in enumerate(D):
    if np.sum(d) == 0.: # if the whole row/col is zero..
        idxs.append(i)
print(f'N removed: {len(idxs)}')
...
plt.tight_layout()
plt.savefig(f'fig/{os.path.basename(fname).split(".")[0]}_binaryHit_continuousCom_noLab_pruning.png')
plt.close()
...
plt.tight_layout()
plt.savefig(f'fig/{os.path.basename(fname).split(".")[0]}_values_sizeHit_noLab_pruning.png')
plt.close()
